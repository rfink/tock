			...
			async.parallel(
				...
				function(err, results) {
					...
					jsonPipe.on('data', function(obj) {
						...
						//   socket clients
						if (obj.stdOut) {
							stdOutStream.write(obj.stdOut, function(err) {
						...
						//   socket clients
							...
							stdErrStream.write(obj.stdErr, function(err) {
								if (err) return errorHandler(errorHandler.ERROR_WARNING, JSON.stringify(err));
							});
							_.forEach(dashboardClients, function(socket) {
								if (socket.jobId != JobModel._id) return;
							...
							JobModel.ErrorCode = obj.errorCode;
							return errorHandler(errorHandler.ERROR_WARNING, obj.errorCode);
					...
					res.on('end', function() {
						...
						stdOutStream.close(function() {});
						stdErrStream.close(function() {});
...
}).listen(config.masterControlPort);
