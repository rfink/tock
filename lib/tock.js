...
var io = require('socket.io').listen(17970);
...
io.sockets.on('connection', function(socket) {
	...
	socket.on('disconnect', function() {
		console.log('Done!  Merf');
		delete dashboardClients[socket.id];
		console.log(dashboardClients);
	});
});
...
var spawnJob = function(job) {
	...
	JobModel.save(function(err) {
		...
		_.forEach(dashboardClients, function(socket) {
			socket.emit('news', {
				'type': 'Job Spawned',
				'message': 'Command ' + job.Command + ' was spawned on host ' + host.host
			});
		});
	});	
};
		...
		var JobModel = Job.findById(jobData._id, function(err, MyJob) {
			...
			MyJob.save(function(err) {
				...
				_.forEach(dashboardClients, function(socket) {
					socket.emit('news', {
						'type': 'Job Finished',
						'message': 'Job ' + MyJob.Command + ' finished'
					});
				});
			});
		});
