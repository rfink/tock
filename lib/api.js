...
app.get('/job/:id/kill', function(req, res, next) {
	...
	var request = http.request({host: 'localhost', port: 16162, method: 'POST'}, function(response) {
...
app.get('/job', function(req, res, next) {
	Job.find().sort([['DateTimRun', 'descending']]).limit(100).run(function(err, results) {
		if (err) return next(err);
		return res.render('job/index.html', results);
	});
});
