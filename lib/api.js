...
var destroyJobClients = {}, syncJobClients = {};
...
tock.listener.on('jobComplete', function(job) {
...
tock.listener.on('stdOut', function(data) {
	_.forEach(io.sockets.clients(), function(dashClient) {
		if (dashClient.get('jobId') != data.id) return;
...
tock.listener.on('stdErr', function(data) {
	_.forEach(io.sockets.clients(), function(dashClient) {
		if (dashClient.get('jobId') != data.id) return;
...
tock.listener.on('spawnJob', function(job) {
	io.sockets.emit('news', {
		'type': 'Job Spawned',
		'jobId': job._id,
		'command': job.Command,
...
// TODO: tock.listener.on('jobError');
tock.listener.on('jobKilled', function(job) {
