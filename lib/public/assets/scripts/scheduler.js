$(function() {
	...
	$('#tock-param-add').click(function() {
		var $el = $('<div class="input-row input-parameters"></div>');
		$el.append($('<label></label>'));
		$el.append($('<input type="text" name="jobSchedule[params][]"></input>'));
		$el.append($('<img src="/assets/images/cross-circle.png" class="tock-param-remove"></img>'));
		$el.insertAfter($(this).parents('div.form').first().find('.input-parameters:last'));
	});
	$('.tock-param-remove').live('click', function() {
		$(this).parents('.input-row:first').remove();
	});
	$('#run-job-now').click(function() {
		if ($(this).is(':checked')) {
			$('#job-date-time').attr('disabled', 'disabled');
		} else {
			$('#job-date-time').removeAttr('disabled');
		}
	});
